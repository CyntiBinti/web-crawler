...
/**
 * @file Given a starting URL, the simple web crawler should visit each URL it finds on the same domain. It should print each URL visited, and a list of links found on that page. The crawler should be limited to one subdomain - so when you start with *https://insert-site.com/*, it would crawl all pages on the insert-site.com website, but not follow external links, for example to facebook.com or community.insert-site.com. Constraints: do not use frameworks like scrapy or go-colly which handle all the crawling behind the scenes or someone else's code, but can use libraries to handle things like HTML parsing.
 */
